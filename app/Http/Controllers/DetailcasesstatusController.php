...
class DetailcasesstatusController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $datas = DB::table('crm_cases')
            ->select(DB::raw(DB::raw('ROW_NUMBER() OVER (ORDER BY cdate ASC) as row_number'),'DATE(crm_cases.created_at) as cdate'), DB::raw('TIME(crm_cases.created_at) as ctime'), 'telno', 'casetype1', 'casedetail', 'casestatus', 'tranferstatus', 'users.name as agent')
