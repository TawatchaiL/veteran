...
class CallstatusController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $datac = DB::connection('remote_connection')
            ...
            ->select('crm_id', DB::raw('count(crm_id) as agentcall'), DB::raw('SUM(if(status = "terminada",1,0)) as terminada') ,DB::raw('SEC_TO_TIME(ROUND(AVG(TIME_TO_SEC(duration_wait)), 0)) as avgwait'), DB::raw('SEC_TO_TIME(SUM(TIME_TO_SEC(duration))) as duration') , DB::raw('SEC_TO_TIME(ROUND(AVG(TIME_TO_SEC(duration)), 0)) as avgduration'))
