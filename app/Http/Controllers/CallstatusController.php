...
class CallstatusController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $dataa = DB::connection('remote_connection')
            ...
            ->select('crm_id as crmid', DB::raw('SEC_TO_TIME(ROUND(SUM(if(id_break is null,duration,0)),0)) as logintime'), DB::raw('SEC_TO_TIME(ROUND(SUM(if(id_break is not null,duration,0)),0)) as breaktime'))
            ->whereRaw('datetime_init between "' . $startDate . ' 00:00:00" and "' . $endDate . ' 23:59:59" and crm_id is not null')
            ->groupBy('crm_id');
            //->get();
