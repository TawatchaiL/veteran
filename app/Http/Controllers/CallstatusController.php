...
class CallstatusController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $datas = DB::connection('remote_connection')
            ...
            ->select('crm_id', DB::raw('count(crm_id) as agentcall'), DB::raw('SUM(if(status = "terminada",1,0)) as terminada') ,DB::raw('ROUND(SEC_TO_TIME(AVG(duration_wait)), 0) as avgwait'), DB::raw('SEC_TO_TIME(SUM(duration)) as duration') , DB::raw('SEC_TO_TIME(AVG(duration)) as avgduration'))
