...
*/        
                    ...
                    ->table(DB::raw("(SELECT @rownumber:=@rownumber + 1 AS rownumber, t.* FROM (SELECT timeslot.timeslot as timelabel, c.*, if(c.numberhour IS NULL,0,c.total_cases) as sumt FROM call_center.timeslot left join (SELECT
                                ...
                                SUM(CASE
                                    ...
                                    WHEN TIME(datetime_init) < '24:00:00' THEN 1 
                                    ELSE 0
                                END) as total_cases FROM call_center.call_entry WHERE LENGTH(callerid) < 6 AND datetime_init between '". $startDate ." 00:00:00' and '". $endDate ." 23:59:59' GROUP BY numberhour) as c on timeslot.timeslot = c.numberhour ORDER BY timeslot.timelabel
