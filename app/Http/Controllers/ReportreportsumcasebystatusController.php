...
class ReportreportsumcasebystatusController extends Controller
{
    /**
    ...
    public function index(Request $request)
    {
        if (!empty($request->get('sdate'))) {
            ...
            $datas = DB::table('crm_cases')
                ->select(DB::raw('ROW_NUMBER() OVER (ORDER BY sumcases DESC) as rownumber'),DB::raw('if(casestatus != "",casestatus,"ไม่ระบุ") as name1'), DB::raw('count(*) as sumcases'))
