  ...
  const _show = () => {
    ...
    switch (settings.type) {
      case "alert":
        keyb = "true";
        backd = "true";
        $("#ezAlerts-footer").html(`<button class="btn btn-${settings.alertType}">${settings.okButtonText}</button>`).on("click", ".btn", function () {
          calbackParam = true;
          $("#ezAlerts").modal("hide");
        });
        break;
      case "confirm":
        let btnhtml = `<button id="ezok-btn" class="btn btn-primary">${settings.yesButtonText}</button>`;
        if (settings.noButtonText && settings.noButtonText.length > 0) {
          btnhtml += `<button id="ezclose-btn" class="btn btn-default">${settings.noButtonText}</button>`;
        }
        ...
        break;
      case "prompt":
        $("#ezAlerts-message").html(settings.messageText + '<br /><br /><div class="form-group"><input type="' + settings.inputFieldType + '" class="form-control" id="prompt" /></div>');
        $("#ezAlerts-footer").html(`<button class="btn btn-primary">${settings.okButtonText}</button>`).on("click", ".btn", function () {
